...
from lib.models.layers.tlpgn_original import TLPGN
...
class VisionTransformer(nn.Module):
    ...
    def forward_self(self, tensor, type, attention, target_token=None):
        ...
        # 保存 patch_embed 层的参数
        # torch.save(self.patch_embed.state_dict(), 'patch_embed_weights_mamba.pth')
    ...
    def forward(self, z, x=None, target_token=None, mode='train', loader_type='normal'):
        if mode == 'z':
            ...
            # x_feature_selected = torch.cat((x_feature[:, :256, :], x_feature[:, 272:, :]), dim=1)
            # torch.save(x_feature_selected, "x_feature_selected_origin.pt")

...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['trackingnet', 'got10k', 'got10k_train_haze', 'got10k_train_dark']:
    ...
    def save_bb(file, data, dataset=""):
        if dataset == 'got10k_train_haze' or dataset == 'got10k_train_dark':
    ...
    for key, data in output.items():
        ...
        #0715
        if key == 'target_bbox':
            if seq.dataset == 'got10k_train_haze' or seq.dataset == 'got10k_train_dark':
        ...
        if key == 'all_scores':
            ...
            if seq.dataset != 'got10k_train_haze' and seq.dataset != 'got10k_train_dark':
    ...
    def _results_exist():
        if seq.object_ids is None:
            if seq.dataset in ['trackingnet', 'got10k']:
                base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
                bbox_file = '{}.txt'.format(base_results_path)
            elif seq.dataset == 'got10k_train_haze' or seq.dataset == 'got10k_train_dark':
                base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
                bbox_file = '{}/pl.txt'.format(base_results_path)
            else:

...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        if extreme_type == 'haze':
            self.pl_path = os.path.join(pl_path, 'pseudo_label/got10k_train_haze') if pl_path != '' else ''
        elif extreme_type == 'dark':
            self.pl_path = os.path.join(pl_path, 'pseudo_label/got10k_train_dark') if pl_path != '' else ''
        else:
            self.pl_path = ''
        ...
        while not valid:
            ...
            try:
                ...
                if dataset.name == 'GOT10k_haze' or dataset.name == 'GOT10k_dark' or dataset.name == 'GOT10k_rainy' or dataset.name == 'GOT10k_snowy':
                    template_frames, template_anno, meta_obj_train, template_frame_paths = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict)

                    search_frames, search_anno, meta_obj_test, search_frame_paths = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)

                # search_frame_paths == img_path
                # 在got-10k-extreme的get_frames中多了下面一句话，使得返回值多了一项
                # img_path = os.path.join(seq_path, '{:08}.jpg'.format(frame_ids[0]+1))
                # 所以search_frame_paths只在got10k-extreme的时候才有，其他时候是None
